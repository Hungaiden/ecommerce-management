...
const FALLBACK_AVATAR =
  "https://ui-avatars.com/api/?name=User&background=6366f1&color=fff&size=40";
...
export default function AdminCartsPage() {
  ...
  const handleRemoveItem = async () => {
    ...
    try {
      const updated = await adminRemoveCartItem(
        removingItem.userId,
        removingItem.itemId,
      );
        ...
        prev.map((c) => {
          const uid =
            c.user_id && typeof c.user_id === "object"
              ? c.user_id._id
              : String(c.user_id);
          return uid === removingItem.userId
  ...
  return (
    <div className="flex flex-col">
      <AdminHeader
        ...
        description={`${totalRows} giỏ hàng`}
      />
          ...
          <div className="overflow-hidden rounded-md border">
            <Table>
              ...
              <TableBody>
                {carts.map((cart) => {
                  const user =
                    cart.user_id && typeof cart.user_id === "object"
                      ? cart.user_id
                      : null;
                  const total = calcCartTotal(cart.items);
                  const userId = user?._id ?? String(cart.user_id);
                  ...
                  return (
                    <TableRow key={cart._id}>
                      ...
                      <TableCell className="text-center">
                        <Badge
                          variant={
                            cart.items.length === 0 ? "secondary" : "default"
                          }
                        >
                        ...
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setClearingName(user?.fullName ?? userId);
                            }}
                          >
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {selectedCart?.user_id &&
                typeof selectedCart.user_id === "object" && (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc muốn xoá toàn bộ giỏ hàng của{" "}
              <strong>{clearingName}</strong>? Hành động này không thể hoàn tác.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Bạn có chắc muốn xoá <strong>{removingItem?.name}</strong> khỏi
              giỏ hàng?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
